ActionView::Base.field_error_proc =
  proc do |html_tag, instance|
    ...
    elements =
      Nokogiri::HTML::DocumentFragment.parse(html_tag).css 'label, ' +
              form_fields.join(', ')
    elements.each do |e|
      if form_fields.include? e.node_name
        # Add the error class directly to the element
        e.add_class('input-error')
        ...
        html =
          if e.get_attribute('type') == 'checkbox'
            ...
            %(#{e}<p class="text-sm text-error mt-1">&nbsp;#{instance.error_message.uniq.to_sentence}</p>)
          else
            # Add a hint error after the field
            %(#{e}<p class="text-sm text-error mt-1">&nbsp;#{instance.error_message}</p>)
          end
